  ...
  return (
    <section id="programme">
      <div className="mx-auto max-w-sm">
        ...
        <div className="relative mt-9 space-y-7 before:absolute before:left-[70px] before:top-2 before:h-[calc(100%-1rem)] before:w-px before:bg-gradient-to-b before:from-transparent before:via-amber-400/60 before:to-transparent">
          {event.program.map((step, index) => (
            ...
            >
              ...
              <div className="pb-3">
                <h3 className="font-serif text-xl italic tracking-wide text-amber-950">{step.title}</h3>
                {step.location && <p className="mt-1 font-serif text-xs uppercase tracking-[0.2em] text-amber-800/65">{step.location}</p>}
                {index < event.program.length - 1 && <div className="mt-6 h-px bg-gradient-to-r from-transparent via-amber-400/35 to-transparent" />}
              </div>
            </motion.div>
          ))}
        </div>
